...
--[[local function SetupVehicleMenu()
    local VehicleMenu = {
        ...
        items = {
            ...
            },
            {
                ...
                icon = 'key',
                type = 'client',
                event = 'qb-vehiclekeys:client:RemoveKeys',
                shouldClose = true,
            },
        }
    }
    ...
    if Vehicle == 0 then                 --fixed for new vehcontrol
        ...
        if IsPoliceOrEMS() and (GetVehicleClass(Vehicle) == 18 or GetVehicleClass(Vehicle) == 14) then 
            ...
            for i = 1, #extras do
                table.insert(ExtrasMenu.items, {
                    id = 'extra'..extras[i],
                    title = 'Extra '..extras[i],
                    icon = 'plus',
                    type = 'client',
                    event = 'qb-radialmenu:client:setExtra',
                    data = { id = 'extra'..extras[i]},
                    shouldClose = true,
                })
            end
            table.insert(VehicleMenu.items, ExtrasMenu)
        end
...
local function SetupVehicleMenu()
    ...
    if Vehicle == 0 then                 --fixed for new vehcontrol
        ...
        local allowedExtras = Config.allowedExtras[modelname]
        if allowedExtras then
            local ExtrasMenu = {
                id = 'extras',
                title = 'Extras',
                icon = 'plus',
            ...
            for i = 1, #allowedExtras do
                table.insert(ExtrasMenu.items, {
                    id = 'extra'..allowedExtras[i],
                    title = 'Extra '..allowedExtras[i],
                    icon = 'plus',
                    type = 'client',
                    event = 'qb-radialmenu:client:setExtra',
                    data = { id = 'extra'..allowedExtras[i]},
                    shouldClose = true,
                })
            end
            table.insert(VehicleMenu.items, ExtrasMenu)
...
RegisterNetEvent('qb-radialmenu:client:setExtra', function(data)
    ...
    if veh ~= nil then
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            local engineHealthPercentage = (engineHealth / maxHealth) * 100
            if engineHealthPercentage < 90 then
                QBCore.Functions.Notify(Lang:t("error.vehicle_damaged"), 'error', 2500)
            else
